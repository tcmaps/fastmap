...
from fastmap.core import Work, Mastermind, Poison
from fastmap.db import check_db, fill_db
from fastmap.worker import MapRequester, MapParser, DataBaser 
...
def init_config():
    ...
    parser.add_argument("--retrys", help="times to retry a request", default=3, type=int)
    ...
    if config.location:
        ...
        log.info('Added %5d cells to scan queue.' % fill_db(config.dbfile, cells))
        del cells, lat, lng
...
def main():
    ...
    log.info('Total %5d cells, %3d Workers, %5d cells each.' % (qqtot, config.minions, n))
...
class BootStrap(Mastermind):
    ...
    def run(self):
      ...
      = Queue(), Queue(), Queue(), Queue()
        ...
        qqtfill = len(cells)
        log.debug('Filling Queue... (%5d cells)' % (qqtfill))
        if len(cells) > 0:
            for cell in cells:
        ...
        for m in xrange(self.config.minions):
            Minions.append(MapRequester(m+1, self.config, RPCq, MapQ, accounts[m]))

        MapT = MapParser(0, self.config, MapQ, SQLq)
        DBt = DataBaser(0, self.config, SQLq, doneQ, locks=[dblock])
        ...
        while dontkillme:
            try: 
                while not doneQ.empty():
                    ...
                    if type(work) is Poison:
                        if killswitch is False: killswitch = None
                        continue
                    
                    if work.work is True:
    
                        log.debug('Cell %10s marked as done.' % work.index); self.ok += 1
                        log.info('Completed %5d of %5d Cells.' % (self.pos+1,qqtot))
    
                    elif work.work is False:
                        logf.error('%s' % work.index); self.fail += 1 
                        log.debug('Cell %10s marked as failed.' % work.index)
                        
                    else: log.debug('Cell %10s ignored.' % work.index); self.pos += 1
                
                self.pos = (self.ok + self.fail)
                log.debug('Cells: %5d total  %5d done %5d failed %5d lost' % (self.pos, self.ok, self.fail,\
                                                                                self.pos - self.ok - self.fail))
                log.debug('Queue: %5d RPC    %5d MAP  %5d DB' % (RPCq.qsize(),MapQ.qsize(),SQLq.qsize()))
                ...
                if not killswitch and RPCq.empty() and qqtfill < qqtot:
                    ...
                    log.debug('Refilled Queue. (%5d cells)' % qqrfill)
                ...
                if killswitch is None:
                    RPCq.put(Poison(relay=True))
                    MapQ.put(Poison(relay=True))
                    SQLq.put(Poison())
                    killswitch = True
                ...
                logf.error(e); log.error(e); continue
